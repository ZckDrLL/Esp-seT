...
class _MasterInfoPageState extends State<MasterInfoPage> {
  ...
  Future<void> _assignIp() async {
    ...
    if (!_isValidIPv4(ip)) {
      await _showResultDialog(
        'Ошибка',
        'Введите корректный IPv4 в формате 192.168.x.x (0-255).',
      );
      return;
    }
    ...
    final result = await widget.manager.assignSlaveIp(
      slaveMac: mac,
      slaveIp: ip,
    );
  ...
  Widget _buildBody() {
    ...
    // Получаем "сырые" MAC из JSON и формируем список STA-MAC
    List<String> normalizeMacList(List<dynamic>? raw) {
      if (raw == null) return <String>[];

      final Set<String> outSet = <String>{};

      for (final item in raw) {
        if (item == null) continue;

        final String mac = item.toString().trim();
        final String macClean = mac.replaceAll(RegExp(r'[^A-Fa-f0-9]'), '');

        if (macClean.length != 12) continue;

        final formatted = List<String>.generate(
          6,
          (i) => macClean.substring(i * 2, i * 2 + 2).toUpperCase(),
          growable: false,
        ).join(':');

        outSet.add(formatted);
      }
    ...
    final List<String> slaves = normalizeMacList(
      (info!['slaves'] as List<dynamic>? ?? []),
    );
    ...
    if (slaveMacController.text.isEmpty && slaves.isNotEmpty) {
      slaveMacController.text =
          slaves.first; // info['slaves'] содержит MAC-строки
    }
          ...
          TextField(
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: const UnderlineInputBorder(
                borderSide: BorderSide(color: Color(0xFF579DDA), width: 2),
              ),
            ),
          ),
