  ...
  class Esp32NatRouterApp extends StatelessWidget {
    ...
    Widget build(BuildContext context) {
      const Color brandBlue = Color(0xFF579DDA); // ваш синий
      ...
      return MaterialApp(
        ...
        theme: ThemeData(
          ...
          colorScheme: ColorScheme.fromSwatch().copyWith(
            primary: const Color(0xFF5EA1D4),
          ),
        ),
        home: HomePage(),
      );
  ...
  class _HomePageState extends State<HomePage> {
    final Esp32Manager esp32Manager = Esp32Manager(
    ...
    Future<void> _primeChatCacheForIp(String ip) async {
      ...
      try {
        await ChatHistoryStore.instance.ensureLoaded(ip);

        final snapshot = await esp32Manager.fetchChatSnapshot();
        if (snapshot != null) {
          await ChatHistoryStore.instance.mergeRemoteSnapshot(ip, snapshot);
        }
      } catch (_) {}
    }
    ...
    Future<void> checkServer() async {
      setState(() {
        isChecking = true;
        statusText = 'Определение IP текущей Wi-Fi сети...';
        serverAvailable = false;
      });

      final available = await esp32Manager.refreshServerFromCurrentWifi();
      final currentIp = esp32Manager.serverIp;

      if (!mounted) return;
      setState(() {
        serverAvailable = available;
        statusText = available ? '' : 'ESP-сервер не доступен';
        isChecking = false;
      });

      if (available && currentIp.isNotEmpty) {
        await _primeChatCacheForIp(currentIp);
      }
    }
        ...
        builder: (_) => AlertDialog(
          ...
          title: ClipRRect(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: const Color(0xFF579DDA)),
              child: Row(
                children: [
                  PopupMenuButton<String>(
                      ...
                      if (value == 'reboot') {
                        ...
                        final ok = await showDialog<bool>(
                          ...
                          builder: (ctx) => AlertDialog(
                            ...
                            content: const Text(
                              'Это действие обнулит все сетевые параметры и перезагрузит устройство. Продолжить?',
                            ),
                            actions: [
                              TextButton(
                                onPressed: () => Navigator.of(ctx).pop(false),
                        ...
                        if (devMode.value) {
                          ...
                          await _showResultDialog(
                            'Недоступно',
                            'Веб-версия недоступна в режиме разработчика',
                          );
                          return;
                        }
                        if (!mounted) return;
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (_) =>
                                WebVersionPage(url: esp32Manager.serverUrl),
                          ),
                        );
                        return;
                        ...
                        Navigator.of(context).push(
                          MaterialPageRoute(
                            builder: (_) => MasterInfoPage(manager: esp32Manager),
                          ),
                        );
                        return;
                      }
                    ...
                    itemBuilder: (ctx) {
                      ...
                      final bool showMasterInfo =
                          (host == '192.168.4.1') || devMode.value;
                        ...
                        Expanded(
                          child: ElevatedButton(
                            onPressed: () async {
                              final ok = await showDialog<bool>(
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  content: const Text(
                                    'Отправить AP SSID и AP Password на устройство?',
                                  ),
                                  actions: [
                                    TextButton(
                                      onPressed: () =>
                            ...
                            style: ElevatedButton.styleFrom(
                              ...
                              foregroundColor: const Color(0xFF579DDA),
                            ),
                            ...
                            onPressed: () async {
                              ...
                              if (ssidValue.isEmpty) {
                                ...
                                await showDialog(
                                  ...
                                  builder: (ctx) => AlertDialog(
                                    ...
                                    content: const Text(
                                      'Поле STA SSID не должно быть пустым.',
                                    ),
                                    actions: [
                                      TextButton(
                              ...
                              final ok = await showDialog<bool>(
                                ...
                                builder: (ctx) => AlertDialog(
                                  ...
                                  content: const Text(
                                    'Отправить STA SSID и STA Password на устройство?',
                                  ),
                                  actions: [
                                    TextButton(
                                      onPressed: () =>
      ...
      return Scaffold(
            ...
            child: ListView(
              children: [
                ...
                SizedBox(
                  width: double.infinity,
                  child: ElevatedButton(
                    onPressed: isChecking ? null : checkServer,
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.white,
                      foregroundColor: const Color(0xFF579DDA),
                    ),
        ...
        floatingActionButton: FloatingActionButton(
          onPressed: () {
            Navigator.of(context).push(
              MaterialPageRoute(
                builder: (_) => MessengerPage(manager: esp32Manager),
              ),
            );
          },
          backgroundColor: const Color(0xFF579DDA),
        ...
        builder: (ctx) {
          return AlertDialog(
            ...
            content: SingleChildScrollView(
              child: Text(_devModeDescription(newVal)),
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.of(ctx).pop(false),
              ...
              TextButton(
                onPressed: () => Navigator.of(ctx).pop(true),
                child: Text(newVal ? 'Включить' : 'Выключить'),
              ),
            ],
          );
        },
                ...
                return ListTile(
                  ...
                  trailing: IconToggleSwitch(
                    ...
                    activeTrackColor: const Color(
                      0xFFD9F1FF,
                    ), // светло-голубой трек ON
                    inactiveTrackColor: const Color(
                      0xFF2F3437,
                    ), // тёмный трек OFF
                    activeThumbInner: const Color(
                      0xFF123A52,
                    ), // тёмный круг внутри ON
                    inactiveThumbInner: const Color(
                      0xFF2F3437,
                    ), // темный круг OFF
                    thumbBorderColorOn: const Color(
                      0xFF9AD1FF,
                    ), // border ON (можно скорректировать)
                    thumbBorderColorOff: const Color(
                      0xFF9EA3A6,
                    ), // border OFF (светлее)
